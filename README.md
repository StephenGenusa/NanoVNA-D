...
* **SD folder browsing** (two levels on the H4, e.g. `CAL/HF/`): folders show as `/NAME` in the
    file browser, `..` goes up a level, the browser's NEW button creates a folder, and **SAVE writes into the folder the browser last showed** — browse into a folder, press X, then save;
...
* **SWR ANT trace format** (DISPLAY→FORMAT→SWR ANT→SWR ANT) — the SWR at the far end of the feedline,
    ...
    reads a better SWR than the antenna has: \|Γ\_ant\| = \|Γ\_meas\|·10^\(L/10\)\. Run SWR and SWR ANT as two
...
* **Workflows (experimental)**
    ...
    * **CHOKE (S21) measure and a fixture null** (MEASURE→MORE (S21)→CHOKE (S21); H4 only): a
        ...
        choke resistance. Each band reports its worst-point series resistance R\_S and a verdict
        ...
        reactance in the common-mode circuit rather than help it, while R\_S only ever adds loss. A
        ...
        leaks back into R\_S — which is what lets a clip-lead jig grade a multi-kΩ choke across the
        ...
        *choke-recipe* guides (below) for the fixture, pass criteria and turns tables.
...
* **Raw S21 phase corrected** (hardware THRU polarity inversion undone in firmware,
    [#81](https://github.com/DiSlord/NanoVNA-D/issues/81)). **Note: re-do your THRU calibration after flashing** — thru data saved by older firmware
...
For boards populated with an MS5351 or SWC5351 clock generator (e.g. HW version 4.3\_MS),
