...
A user manual is being built in `docs/manual/` from the firmware source. Generated so far: the
menu map (`09-menu-map.md`, every menu with a mockup), the console command reference
(`08-console.md`) and the trace-format table (`02-trace-formats.md`); `make -C docs/manual all`
regenerates them and `python3 -m unittest tests.test_manual_gen` checks them against the source.
