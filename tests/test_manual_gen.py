...
class MenuParserTests(unittest.TestCase):
    ...
    def test_counts_match_source(self):
        ...
        self.assertEqual(len(m4), 48)  # definitions only; forward declarations excluded
...
class GenMenusTests(unittest.TestCase):
    def test_generate(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(md.count("\n## "), 47)                          # one section per reachable table
            svgs = [f for f in os.listdir(os.path.join(tmp, "img")) if f.startswith("menu-") and f.endswith(".svg")]
            self.assertEqual(len(svgs), 90)                                  # 47 H4 + 43 H
            self.assertEqual(len([f for f in svgs if f.endswith("-H4.svg")]), 47)
