...
class MenuParserTests(unittest.TestCase):
    ...
    def test_counts_match_source(self):
        ...
        self.assertEqual(len(m4), 47)  # definitions only; forward declarations excluded
        ...
        self.assertEqual(self.h4.count("{ MT_"), 343)
    ...
    def test_top_and_continuation(self):
        ...
        self.assertTrue(any(i.label == "\x1b SWR ANT" and i.kind == "submenu" for i in fmt.items))   # SWR ANT submenu
        self.assertTrue(any(i.label == "SWR ANT" for i in m4["menu_swr_ant"].items))
...
class GenMenusTests(unittest.TestCase):
    def test_generate(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(md.count("\n## "), 46)                          # one section per reachable table
            svgs = [f for f in os.listdir(os.path.join(tmp, "img")) if f.startswith("menu-") and f.endswith(".svg")]
            self.assertEqual(len(svgs), 89)                                  # 46 H4 + 43 H
            self.assertEqual(len([f for f in svgs if f.endswith("-H4.svg")]), 46)
