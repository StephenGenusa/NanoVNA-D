...
level. Two levels are supported on the H4 (`CAL/HF/…`), the browser's NEW button creates a
folder in place, and saves go into the folder last browsed (chapter 7 gives the steps). On the
H folders are a build option
