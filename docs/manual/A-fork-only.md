...
| Ch. 7 | folder browsing (`/NAME`, `..`; two levels on the H4), `.nvs` scripts, the on-device guide viewer (SD CARD → LOAD → GUIDE, H4; H opt-in) with the shipped `GUIDES` pack, and PNG screenshots (H4; IMAGE FORMAT cycles BMP / TIFF / PNG, LOAD SCREENSHOT lists all three) | flat file list; `.cmd` only; no guides; BMP / TIFF only |
| Ch. 8 | `*IDN?` / `*idn?`; `trace N swrant` | absent |
| Ch. 9, menu map | SD CARD → LOAD → GUIDE (H4); DISPLAY → SCALE → HAM BANDS; DISPLAY → FORMAT → SWR ANT, CABLE LOSS, CABLE TYPE (H4), CABLE LENGTH (H4); STIMULUS → MUTE OUTPUT ON PAUSE; MEASURE → SWR BW (S11) (H4) | absent |
