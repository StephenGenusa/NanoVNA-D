...
| Ch. 7 | one-level folder browsing (`/NAME`, `..`), `.nvs` scripts, and the on-device guide viewer (SD CARD → LOAD → GUIDE, H4; H opt-in) with the shipped `GUIDES` pack | flat file list; `.cmd` only; no guides |
