...
| Ch. 7 | one-level folder browsing (`/NAME`, `..`), `.nvs` scripts, the on-device guide viewer (SD CARD → LOAD → GUIDE, H4; H opt-in) with the shipped `GUIDES` pack, and PNG screenshots (H4; IMAGE FORMAT cycles BMP / TIFF / PNG, LOAD SCREENSHOT lists all three) | flat file list; `.cmd` only; no guides; BMP / TIFF only |
